...
async def batch(client: Client, message: Message):
    ...
    try:
        # Ask the user to provide the second batch link (starting point)
        start_link_msg = await client.ask(
            chat_id=message.from_user.id,
            text="Send the second batch link (starting point) from the DB Channel.",
            filters=(filters.text & ~filters.forwarded),
            timeout=60
        )
        start_msg_id = int(start_link_msg.text.split("/")[-1])

        # Calculate the first and last message IDs for Phase 2 batch
        first_batch2_msg_id = start_msg_id + 1  # Start right after the provided message
        last_batch2_msg_id = start_msg_id + total_messages  # Last message based on total messages generated
